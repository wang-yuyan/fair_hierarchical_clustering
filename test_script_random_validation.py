...
def test(filename, num_list, b, r, output_direc, num_instances, rho):
    ...
    obj_f.write("avlk_obj  fair_avlk_obj  fair/unfair avlk ratio  upper bound\n")
    ...
    for num in num_list:
        ...
        for i in range(num_instances):
            ...
            fairlets, swap_counter, random_counter, random_fairlets = find_eps_local_opt_random(blue_pts_sample, red_pts_sample, dist, d_max, b, r, 0.1, rho)

            end = time.time()
            dist_ratio  = float(calculate_obj(random_fairlets, dist) / total_dist)
            ...
            #validate random fairlets instead
            fair_root = avlk_with_fairlets(dist, random_fairlets)
...
if __name__ == "__main__":
    ...
    np.random.seed(0)
    random.seed(0)
    output_direc = "./experiments_random_swap_random_validation_rho_0"
